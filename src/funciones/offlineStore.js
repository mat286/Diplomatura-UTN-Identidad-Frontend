...
export async function DeleteImageOffline(key) {
    // Si solo la usamos para limpieza, podemos simplificar:
    await localforage.removeItem(key);
 ...
 */
export async function deletePersonFromLocalStorage(hashIdentidadToRemove) { // <--- CLAVE: AÑADIR ASYNC
    const STORAGE_KEY = 'peopleList';
    const cleanPromises = []; // Para manejar la eliminación asíncrona de imágenes
